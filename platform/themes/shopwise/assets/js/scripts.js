...
(function ($) {
    ...
    *===================================*/
    ...
    $(document).on("input", ".text--countable", function (e) {
        var len   = $(this).data('limit');
        var input = $(this).find('textarea');
        if(!$(this).find('span.char-limit').length) {
            $(this).append('<span class="char-limit"></span>')
        }
        var inputLength = input.val().length;
        if (inputLength >= len) {
            input.val(input.val().substring(0, len));
            $(this).find('.char-limit').text(inputLength + ' characters left');
            if(!$(this).find('.char-limit').hasClass('red')) $(this).find('.char-limit').addClass('red');
        } else {
            $(this).find('.char-limit').text(len - inputLength + ' characters left');
        }
      }
    );

    $(document).on("change", ".file-upload--avatar input", function(evt) {
        var fileUrl = URL.createObjectURL(this.files[0]);
        $(".file-upload--avatar img").attr("src", fileUrl);
      });

    $(document).on("change", ".file-upload--video input", function(evt) {
        var $source = $('.file-upload--video video source');
        $source[0].src = URL.createObjectURL(this.files[0]);
        $source.parent()[0].load();
      });

      $('.header_list .dropdown-toggle').off('click');

      $(document).on('click','.header_list .dropdown-toggle', function (e) {
    ...
    $('.toggle--header input[type="checkbox"]').on('change', function () {
    ...
    *===================================*/
    ...
    $('.toggle_video_modal').each(function () {
        $(this).magnificPopup({
            type: 'inline',
    ...
    *===================================*/
    ...
    /*==============================================================
    13A. UPLOAD SCRIPTS JS
    ==============================================================*/

    if ($("#request--video-upload").length > 0) {
        // console.log($("#request--video-upload"));
        const uppy = new Uppy({
            autoProceed: false,
            ...
            // .use(Tus, { endpoint: $("#request--video-upload").data('action') })
            .use(XHRUpload, {
                endpoint: $("#request--video-upload").data('action'),
                formData: true,
                headers: { 'X-CSRF-TOKEN': $("#request--video-upload").data('token') },
                fieldName: 'video',
                method: 'post',
