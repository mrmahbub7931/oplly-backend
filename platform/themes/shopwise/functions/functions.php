...
register_sidebar([
    ...
    'description' => __('Sidebar on the product page'),
]);

register_sidebar([
    'id'          => 'product_checkout',
    'name'        => __('Checkout Pages area'),
    'description' => __('Area after checkout content'),
]);

register_sidebar([
    'id'          => 'product_booklive',
    'name'        => __('Book Live after Content'),
    'description' => __('Area after Book live content'),
]);
...
theme_option()
    ->setField([
        ...
        'attributes' => [
            'name'    => 'copyright',
            'value'   => '© 2020 Oplly Technologies. All right reserved.',
            'options' => [
                'class'        => 'form-control',
    ...
    ->setSection([
        'title'      => __('Watermark'),
        'desc'       => __('Options for managing watermarks'),
        'id'         => 'opt-text-subsection-watermarkr',
        'subsection' => true,
        'icon'       => 'fas fa-magic',
    ])
    ->setField([
        'id'         => 'logo_watermark',
        'section_id' => 'opt-text-subsection-watermarkr',
        'type'       => 'mediaImage',
        'label'      => __('Watermark Logo'),
        'attributes' => [
            'name'  => 'logo_watermark',
            'value' => null,
        ],
...
if (is_plugin_active('ecommerce')) {
    ...
    add_shortcode(
        'become-talent',
        __('Become Talent Banner'),
        __('Become Talent Banner'),
        function ($shortCode) {

            return Theme::partial('short-codes.become-talent', [
                'title'       => $shortCode->title,
                'description' => $shortCode->description,
            ]);
        }
    ...
    /* shortcode()->setAdminConfig(
        'become-talent',
        Theme::partial('short-codes.become-talent-admin-config')
    ); */
    ...
    add_shortcode('products-by-category', __('Products by Category'), __('Products by Category'), function ($shortCode) {
        $title = ProductCategory::find($shortCode->category);
        $title = strlen($shortCode->title) > 0 ? $shortCode->title : $title->name ?? '';
        return Theme::partial('short-codes.products-by-category', [
            'title'    => $title,
    ...
    shortcode()->setAdminConfig('products-by-category', Theme::partial('short-codes.products-by-category-admin-config', [
...
add_shortcode('become-talent-form', __('Talent Banner'), __('Talent Banner'), function ($shortCode) {
    return Theme::partial('short-codes.become-talent-form', [
        'title'       => $shortCode->title,
        'description' => $shortCode->description,
    ]);
});

shortcode()->setAdminConfig('become-talent-form', Theme::partial('short-codes.become-talent-form-admin-config'));
...
if (is_plugin_active('contact')) {
    add_filter('contact-form-template-view', function () {
        return Theme::getThemeNamespace() . '::partials.short-codes.contact-form';
    }, 120);

    add_shortcode('contact-icons', __('Contact Page Icons'), __('Contact Page Icons'), function ($shortCode) {
        return Theme::partial('short-codes.contact-icons', []);
    });

    add_shortcode('contact-map', __('Contact Page Map'), __('Contact Page Map'), function ($shortCode) {
        return Theme::partial('short-codes.contact-map', []);
    });
}
