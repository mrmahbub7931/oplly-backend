...
class ProductRepository extends RepositoriesAbstract implements ProductInterface
{
    ...
    public function getProductsWithCategory(array $params)
    {
        ...
        $this->model = $this->model
            ...
            ->where(function ($query) use ($params) {
                ...
                return $query
                    ->whereIn(
                        'ec_product_categories.' . $params['categories']['by'],
    ...
    public function getProductsByCollections(array $params)
    {
        ...
        $this->model = $this->model
            ...
            ->where(function ($query) use ($params) {
                ...
                return $query
                    ->whereIn(
                        'ec_product_collections.' . $params['collections']['by'],
    ...
    public function filterProducts(array $filters, array $params = [])
    {
        ...
        // Filter product by allow_speed_service
        if ($filters['allow_speed_service']) {
            $this->model = $this->model
                ->where(function ($query) use ($filters) {
                    /**
                     * @var Builder $query
                     */
                    $query = $query->whereHas('owner', function ($q) {
                        $q->where('allow_speed_service', true);
                    });
                    return $query;
                });
        }

        // Filter product by rating5
        if ($filters['rating5']) {
            $this->model = $this->model
                ->where(function ($query) use ($filters) {
                    /**
        ...
        if ($filters['tags']) {
            $this->model = $this->model
                ->leftJoin(
                    ...
                    'ec_product_tag_product.product_id',
                    '=',
                    'ec_products.id'
                )
                ->where(function ($query) use ($filters) {
