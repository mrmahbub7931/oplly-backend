...
use Canopy\Ecommerce\Http\Requests\ReviewRequest;
...
class PublicProductController
{
    ...
    public function getProductTag($slug, Request $request, ProductTagInterface $tagRepository)
    {
        ...
        $tag = $tagRepository->getFirstBy(['id' => $slug->reference_id], ['*'], ['products']);
    ...
    public function getAllProductCategories(
        ...
        $categories = $categoryRepository->getCategories(['active' => true]);
        $featuredCategories = $categoryRepository->getFeaturedCategories(-1);

        Theme::breadcrumb()
            ->add(__('Home'), url('/'))
            ->add(__('Categories'), route('public.categories'));
        //->add($category->name, $category->url);

        // do_action(BASE_ACTION_PUBLIC_RENDER_SINGLE, PRODUCT_CATEGORY_MODULE_SCREEN_NAME, $categories);

        return Theme::scope(
            'ecommerce.product-categories',
            compact('featuredCategories', 'categories'),
            'plugins/ecommerce::themes.product-categories'
        )->render();
    }
    ...
    public function postCreateReview(ReviewRequest $request, BaseHttpResponse $response)
    {
        ...
        /* Checking if the multiple review feature is enabled. */ 
        if (check_mutiple_review_feature() != true) { 
            if ($exists > 0) { 
                return $response 
                    ->setError() 
    ...
    public function getBrand($slug, Request $request, GetProductService $getProductService)
    {
        ...
        $brand = $this->brandRepository->findById($slug->reference_id);
