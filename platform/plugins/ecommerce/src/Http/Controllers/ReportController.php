...
class ReportController extends BaseController
{
    ...
    public function getIndex(TopSellingProductsTable $topSellingProductsTable)
    {
        ...
        $count['refunds'] = $this->orderRepository
            ->getModel()
            ->join('payments', 'payments.order_id', '=', 'ec_orders.id')
            ->where('payments.status', PaymentStatusEnum::REFUNDED)
            ->sum('sub_total');

        $count['refundsMonth'] = $this->orderRepository
            ->getModel()
            ->whereBetween('ec_orders.created_at', [now()->startOfMonth()->toDateString(), now()->toDateString()])
            ->join('payments', 'payments.order_id', '=', 'ec_orders.id')
            ->where('payments.status', PaymentStatusEnum::REFUNDED)
        ...
        $count['refundsToday'] = $this->orderRepository
            ->getModel()
            ->whereBetween('ec_orders.created_at', [now()->startOfDay()->toDateString(), now()->toDateString()])
            ->join('payments', 'payments.order_id', '=', 'ec_orders.id')
            ->where('payments.status', PaymentStatusEnum::REFUNDED)
        ...
        $count['incompleteOrders'] = $this->orderRepository
            ->getModel()
            // ->whereDate('created_at', now()->toDateString())
            ->where('is_finished', '0')
            ->count();

        $count['ordersCount']['accepted'] = $this->orderRepository
            ->getModel()
            ->where('status', 'accepted')
            ->where('is_finished', '1')
            ->count();
        $count['ordersCount']['pending'] = $this->orderRepository
            ->getModel()
            ->where('status', 'pending')
            ->where('is_finished', '1')
            ->count();
        $count['ordersCount']['rejected'] = $this->orderRepository
            ->getModel()
            ->where('status', 'rejected')
            ->where('is_finished', '1')
            ->count();
        $count['ordersCount']['cancelled'] = $this->orderRepository
            ->getModel()
