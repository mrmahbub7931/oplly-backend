...
use Canopy\Ecommerce\Forms\AddShippingRegionForm;
use Canopy\Ecommerce\Http\Requests\AddShippingRegionRequest;
use Canopy\Ecommerce\Http\Requests\ShippingRuleRequest;
...
class ShippingMethodController extends BaseController
{
    ...
    public function postSaveMethods(Request $request, BaseHttpResponse $response, SettingStore $settingStore)
    {
        ...
        if (get_shipping_setting('status', $type) != $request->input('status') ||
            ...
            if (!$hash) {
                $settingStore->set(
                    config('plugins.ecommerce.shipping.settings.prefix') . $type . '_web_hook_hash',
                    md5(Str::random(32))
                );
            }
        }
        ...
        foreach ($data as $settingKey => $settingValue) {
            $settingStore->set(
                config('plugins.ecommerce.shipping.settings.prefix') . $type . '_' . $settingKey,
                $settingValue
            );
        }
    ...
    {
        $settingStore->set(
            config('plugins.ecommerce.shipping.settings.prefix') . $request->input('type') . '_status',
            0
        );
