...
use Canopy\Ecommerce\Forms\ProductForm;
use Canopy\Ecommerce\Http\Requests\CreateProductWhenCreatingOrderRequest;
use Canopy\Ecommerce\Http\Requests\ProductRequest;
use Canopy\Ecommerce\Http\Requests\ProductUpdateOrderByRequest;
use Canopy\Ecommerce\Http\Requests\ProductVersionRequest;
...
use Canopy\Ecommerce\Services\Products\CreateProductVariationsService;
use Canopy\Ecommerce\Services\Products\StoreAttributesOfProductService;
use Canopy\Ecommerce\Services\Products\StoreProductService;
use Canopy\Ecommerce\Services\StoreProductTagService;
use Canopy\Ecommerce\Tables\ProductTable;
use Exception;
...
class ProductController extends BaseController
{
    ...
    public function postAddVersion(
        ...
        if ($addedAttributes && !empty($addedAttributes) && is_array($addedAttributes)) {
            ...
            if (!$result['created']) {
                return $response
                    ->setError()
                    ->setMessage(trans('plugins/ecommerce::products.variation_existed'));
            }

            $this->postSaveAllVersions([$result['variation']->id => $request->input()], $productVariation, $id, $response);

            return $response->setMessage(trans('plugins/ecommerce::products.added_variation_success'));
        }

        return $response
            ->setError()
            ->setMessage(trans('plugins/ecommerce::products.no_attributes_selected'));
    }
    ...
    public function postUpdateVersion(
        ...
        if ($addedAttributes && !empty($addedAttributes) && is_array($addedAttributes)) {
            ...
            return $response->setMessage(trans('plugins/ecommerce::products.updated_variation_success'));
        }
    ...
    public function postGenerateAllVersions(
        ...
        return $response->setMessage(trans('plugins/ecommerce::products.created_all_variation_success'));
    }
    ...
    public function postStoreRelatedAttributes(
        ...
        return $response->setMessage(trans('plugins/ecommerce::products.updated_product_attributes_success'));
    }
