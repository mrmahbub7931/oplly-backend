...
class PublicController extends Controller
{
    ...
    public function postChangePassword(UpdatePasswordRequest $request, BaseHttpResponse $response)
    {
        ...
        if ($mailer->templateEnabled('customer_password_changed')) {
    ...
    public function getListOrders(Request $request)
    {
        ...
        $orders = $this->orderRepository->getModel()
            ->where('user_id', auth('customer')->user()->getAuthIdentifier())
            ->where('is_finished', 1)->orderBy('created_at', 'desc')
            ->with(['products.product', 'talent', 'user'])
            ->paginate(10);
    ...
    public function getModifyOrder($id)
    {
        SeoHelper::setTitle(__('Modify Request :id', ['id' => get_order_code($id)]));
        ...
        Theme::breadcrumb()->add(__('Home'), url('/'))->add(
            __('Modify Request :id', ['id' => get_order_code($id)]),
            route('customer.orders.modify', $id)
        );

        return Theme::scope(
            'ecommerce.customers.orders.modify',
            compact('order', 'occasions'),
            'plugins/ecommerce::themes.customers.orders.modify'
        )->render();
    }


    public function postModifyOrder($id, Request $request, BaseHttpResponse $response)
    {
        ...
        $mailer = EmailHandler::setModule(ECOMMERCE_MODULE_SCREEN_NAME);
        if ($mailer->templateEnabled('customer_modify_order')) {
            OrderHelper::setEmailVariables($order);
            $mailer->sendUsingTemplate(
                'customer_modify_order',
                $order->talent->email ?: $order->address->email
            );
        }

        return $response
            ->setNextUrl(route('customer.orders.view', $id))
            ->setMessage(__('Request updated successfully!'));
    }
