...
Route::group(['namespace' => 'Canopy\Ecommerce\Http\Controllers', 'middleware' => ['web', 'core']], function () {
    Route::group(['prefix' => BaseHelper::getAdminPrefix() . '/ecommerce', 'middleware' => 'auth'], function () {
        ...
        Route::group(['prefix' => 'occasions', 'as' => 'occasions.'], function () {
            Route::resource('', 'OccasionController')
                ->parameters(['' => 'occasion']);

            Route::delete('items/destroy', [
                'as'         => 'deletes',
                'uses'       => 'OccasionController@deletes',
                'permission' => 'occasions.destroy',
            ]);
        });
...
Route::group(['namespace' => 'Canopy\Ecommerce\Http\Controllers\Fronts', 'middleware' => ['web', 'core']], function () {
    Route::group(apply_filters(BASE_FILTER_GROUP_PUBLIC_ROUTE, []), function () {
        ...
        Route::get(SlugHelper::getPrefix(Product::class, 'product') . '/{slug}', [
            'uses' => 'PublicProductController@getProduct',
        ]);

        /* Route::get(SlugHelper::getPrefix(Product::class, 'talent') . '/{slug}', [
            'uses' => 'PublicTalentController@getTalent',
        ]); */

        Route::get(SlugHelper::getPrefix(ProductCategory::class, 'categories'), [
            'as'   => 'public.categories',
            'uses' => 'PublicProductController@getAllProductCategories',
        ]);
