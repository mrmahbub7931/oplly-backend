...
$selectedAttrs = app(\Canopy\Ecommerce\Repositories\Interfaces\ProductVariationInterface::class)
    ...
    <section id="product-detail-page" class="content-page single-product-content">
                ...
                <div class="col-lg-6 col-md-6 col-sm-12 mb-30">
                    <div class="product-page-image">
                        ...
                        <div class="product-image-slider product-image-gallery" id="product-image-gallery" data-pswp-uid="3">
                            ...
                            <div class="item">
                                <img src="{{ RvMedia::getImageUrl($img, 'product_detail') }}" data-zoom-image="{{ RvMedia::getImageUrl($img, 'product_detail') }}" alt="{{ $originalProduct->name }}" />
                            </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="product-image-slider-thumbnails">
                        ...
                        <div class="item">
                            <img src="{{ RvMedia::getImageUrl($thumb,'product') }}" alt="{{ $originalProduct->name }}" />
                        </div>
                        @endforeach
                    </div>
                ...
                <div class="col-lg-6 col-md-6 col-sm-12 mb-30">
                    <div class="product-page-content">
                        ...
                        <div class="product-rating">
                            <div class="star-rating" itemprop="reviewRating" itemscope="" itemtype="http://schema.org/Rating" title="Rated {{ get_average_star_of_product($product->id) }} out of 5">
                                <span style="width: {{ get_average_star_of_product($product->id) * 20 }}%"></span>
                            </div>
                            <div class="product-rating-count"><a href="#list-reviews">( <span class="count">{{ get_count_reviewed_of_product($originalProduct->id) }}</span>
                                    {{ __('Reviews') }} )</a>
                            </div>
                        </div>
                        @endif
                        ...
                        <form class="single-variation-wrap">
                            ...
                            <div class="product-quantity">
                                ...
                                <input class="quantity input-lg" step="1" min="1" max="20" name="qty" value="1" title="Quantity" type="number" />
                                <span data-value="-" class="quantity-btn quantityMinus"></span>
                            </div>
                            <button id="btn-add-cart" class="btn btn-lg btn-black"><i class="fa fa-shopping-bag" aria-hidden="true"></i>{{ __('Add to cart') }}
                            </button>
                        </form>
                        <div class="product-meta">
                            @if ($originalProduct->sku)
                            <span>{{ __('SKU') }} : <span id="product-sku" class="sku" itemprop="sku">{{ $originalProduct->sku }}</span></span>
                            @endif
                            <span><span id="is-out-of-stock">{{ ! $originalProduct->isOutOfStock() ? __('In stock') : __('Out of stock') }}</span></span>
        ...
        <div class="product-tabs-wrapper container">
            <ul class="product-content-tabs nav nav-tabs" role="tablist">
                ...
                <li class="nav-item">
                    <a class="" href="#tab_additional_information" role="tab" data-toggle="tab">{{ theme_option('product-bonus-title')}}</a>
                </li>
                @if (EcommerceHelper::isReviewEnabled())
                <li class="nav-item">
            ...
            <div class="product-content-Tabs_wraper tab-content container">
                ...
                <div id="tab_additional_information" role="tabpanel" class="tab-pane fade">
                    <!-- Accordion Title -->
                    <h6 class="product-collapse-title" data-toggle="collapse" data-target="#tab_additional_information-coll">{{ theme_option('product-bonus-title')}}</h6>
                    <!-- End Accordion Title -->
                    <!-- Accordion Content -->
