            ...
            <div class="col-12 col-lg-8 mx-auto">
                ...
                <div class="form-checkout">@if (isset($products) && $products)
                        ...
                        <div class="talent--pic">
                            <img src="{{ RvMedia::getImageUrl($product->owner->photo, 'thumb', false, RvMedia::getDefaultImage()) }}"
                                 alt="{{ $product->name }}">
                        </div>
                    ...
                    <form action="{{ route('payments.checkout') }}" method="post">
                        ...
                        <div class="form-group  @if ($errors->has('target_audience')) has-error @endif">
                            <div class="btn-group pills--selector" role="group" aria-label="target_audience"
                                 id="target_audience_select">
                                <input type="radio" class="btn-check" name="target_audience" id="target_audience_him" autocomplete="off"
                    ...
                    <span>
                        <i class="fas fa-user"></i>
                                ...
                                <input type="radio" class="btn-check" name="target_audience" id="target_audience_self" autocomplete="off"
                                       value="self">
                    ...
                    <span>
                        <i class="fas fa-star"></i>
                                    ...
                                    For Me</label>
                                <input type="hidden" id="default_price" name="price"
                                       value="{{ $product->owner->price }}">
                                ...
                                @if ($product->owner and $product->owner->allow_business)
                                    ...
                                    <input type="radio" class="btn-check" name="target_audience" id="target_audience_corporate"
                        ...
                        <i class="fas fa-briefcase"></i>
                                    ...
                                    <input type="hidden" id="business_price" name="business_price"
                                           value="{{ $product->owner->business_price }}">
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="form-group mb-3 nice--input @if ($errors->has('recepient')) has-error @endif">
                            ...
                            <input type="text" name="recepient" id="recepient" class="form-control address-control-item checkout-input"
                                   value="{{ old('recepient"', Arr::get($sessionCheckoutData, 'recepient"')) }}">
                        ...
                        <div class="form-group mb-3 hide_on_self nice--input  @if ($errors->has('from')) has-error @endif">
                            ...
                            <input type="text" name="from" id="from" class="form-control address-control-item checkout-input"
                                   value="{{ old('from', Arr::get($sessionCheckoutData, 'from')) }}">
                                ...
                                @foreach ($occasions as $occasion)
                                    <li class="@if($occasion->show_standard) show--default @endif @if($occasion->show_business) show--business @endif">
                                        <label class="btn" for="occasion_{{ $occasion->id }}">
                                            <input type="radio" class="btn-check" name="occasion_id" id="occasion_{{ $occasion->id }}"
                                                   autocomplete="off" value="{{ $occasion->id }}">
                                            <img src="{{ RvMedia::getImageUrl($occasion->image, null, false, RvMedia::getDefaultImage()) }}"
                                                 alt="{{ $occasion->name }}" />
                        ...
                        <div class="form-group mb-3">
                            <div class="chek-form">
                                <div class="custome-checkbox">
                                    ...
                                    <label class="form-check-label text-muted"
                                           for="make_public-me"><span>Hide this video from @if (!empty($product))
                                                {{ $product->name }}'s
                        ...
                        <div class="mb-4">
                            ...
                            <input type="hidden" name="amount"
                                   value="{{ Cart::instance('cart')->rawTotal() - $promotionDiscountAmount - $couponDiscountAmount + $shippingAmount }}">
                            ...
                            <ul class="list-group list_payment_method">
                                @if (strtoupper($currency) != 'BDT' && setting('payment_stripe_status') == 1)
                                    <li class="list-group-item">
                                        <input class="magic-radio js_payment_method" type="radio" name="payment_method" id="payment_stripe"
                                               value="stripe" @if (!setting('default_payment_method') || setting('default_payment_method') == \Canopy\Payment\Enums\PaymentMethodEnum::STRIPE) checked @endif data-toggle="collapse" data-target=".payment_stripe_wrap"
                      ...
                      setting('default_payment_method')==\Canopy\Payment\Enums\PaymentMethodEnum::STRIPE) show @endif">
                          ...
                          $errors->has('expiry') || $errors->has('cvc')) has-error @endif">
                                                    <div class="row">
                                                        ...
                                                        <div class="form-group col-sm-7 nice--input px-1 mb-3">
                                                        ...
                                                        <div class="form-group col-6 col-sm-2 nice--input px-1">
                                ...
                                @if (setting('payment_paypal_status') == 1)
                                    <li class="list-group-item">
                                        <input class="magic-radio js_payment_method" type="radio" name="payment_method" id="payment_paypal"
                                               @if (setting('default_payment_method') == \Canopy\Payment\Enums\PaymentMethodEnum::PAYPAL) checked @endif value="paypal">
                                        <label for="payment_paypal"
                                               class="text-left">{{ setting('payment_paypal_name', trans('plugins/payment::payment.payment_via_paypal')) }}</label>
                                    </li>
                                @endif

                                {!! apply_filters(PAYMENT_FILTER_ADDITIONAL_PAYMENT_METHODS, null, ['amount' => Cart::instance('cart')->rawTotal() - $promotionDiscountAmount - $couponDiscountAmount + $shippingAmount, 'currency' => strtoupper(get_application_currency()->title), 'name' => null]) !!}

                                @if (setting('payment_cod_status') == 1)
                                    <li class="list-group-item">
                                        <input class="magic-radio js_payment_method" type="radio" name="payment_method" id="payment_cod" @if (setting('default_payment_method') == \Canopy\Payment\Enums\PaymentMethodEnum::COD) checked @endif value="cod" data-toggle="collapse"
                                               data-target=".payment_cod_wrap" data-parent=".list_payment_method">
                                ...
                                @if (setting('payment_bank_transfer_status') == 1)
                                    <li class="list-group-item">
                                        <input class="magic-radio js_payment_method" type="radio" name="payment_method"
                                               id="payment_bank_transfer" @if (setting('default_payment_method') == \Canopy\Payment\Enums\PaymentMethodEnum::BANK_TRANSFER) checked @endif value="bank_transfer" data-toggle="collapse"
                                               data-target=".payment_bank_transfer_wrap" data-parent=".list_payment_method">
                            ...
                            <div class="mb-4">
                                ...
                                <ul class="list-group list_delivery_method">
                                    <li class="list-group-item">
                                        ...
                                        <label for="delivery_free"
                                               class="text-left">Standard <b class="text-warning">FREE</b></label>
                                    </li>
                                    <li class="list-group-item">
                                        <input class="magic-radio js_delivery_method" type="radio" name="delivery_method" id="delivery_speed" value="speed">
                                        <label for="delivery_speed"
                                               class="text-left">Speed Service <b class="text-warning">+{{ format_price(Cart::instance('cart')->rawSubTotal() * 0.4) }}</b></label>
                                    </li>
                                </ul>
                            </div>
                        ...
                        @endif
                        <br>
                        <div class="col-12 mx-auto d-md-block" data-price="{{ Cart::instance('cart')->rawTotal() }}" id="main-checkout-product-info">
                                ...
                                @if ($couponDiscountAmount > 0)
                                    <div class="row price discount-amount">
                                        ...
                                        <div class="col-lg-5 col-md-4 col-5">
                                            ...
                                            <p class="price-text total-discount-amount-text" data-discount="{{$couponDiscountAmount}}">
                                ...
                                <div class="row total-price">
                                    ...
                                    <div class="col-lg-5 col-md-4 col-5 float-right">
                                        <p class="total-text raw-total-text" data-price="{{ Cart::instance('cart')->rawTotal() }}">
                                            {{ format_price(Cart::instance('cart')->rawTotal() - $promotionDiscountAmount - $couponDiscountAmount + $shippingAmount) }}
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="form-group">
                            <div class="row">
                                ...
                                <div class="col-12 col-md-8 mx-auto mb-3">
                                    <button type="submit"
                                            class="btn payment-checkout-btn payment-checkout-btn-step btn-primary btn-block float-right"
                                ...
                                <div class="col-12 text-center text-muted pt-3">By booking, you agree to our
        ...
        (function($) {
            ...
            function updateCheckoutNumbers() {
                ...
                if ($('.price.discount-amount').length) {
                    ...
                    $('.total-discount-amount-text').html('- £' + discountedAmount.toFixed(2));
                    newPrice = newPrice - discountedAmount;
                }

                $('.service-price-text').html('£' + servicePrice.toFixed(2));
                $('.price .sub-total-text').html('£' + total.toFixed(2));
