...
<div class="next-form-section">
    <div class="next-form-grid">
        <div class="next-form-grid-cell">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="text-left width-300-px">
                                <a class="text-underline wordwrap" href="{{ route('products.edit', $product->original_product->id) }}" target="_blank" title="{{ $orderProduct->product_name }}">{{ $orderProduct->product_name }}</a>
                            </td>
                            ...
                            <td class="text-right">
                                ...
                                <input class="j-refund-quantity width-50-px pl5 m-auto next-input p-none-r" name="products[{{ $orderProduct->product_id }}]" type="number" min="0" value="{{ $orderProduct->qty - $orderProduct->restock_quantity }}" />
                                @endif
                            </td>
                        </tr>
                        @endif
                ...
                <table class="refund-totals">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-right quantity text-no-bold">
                                @if ($order->payment->status == \Canopy\Payment\Enums\PaymentStatusEnum::PENDING)
                                <span>{{ format_price(0) }}</span>
                                @else
                                <span>{{ format_price($order->amount - $order->payment->refunded_amount) }}</span>
                                @endif
                            </td>
                        </tr>
                    </tbody>
            ...
            @if ($order->payment->status !== \Canopy\Payment\Enums\PaymentStatusEnum::PENDING && ($order->payment->amount - $order->payment->refunded_amount) > 0)
                        ...
                        <tr>
                            ...
                            <td class="p-xs width-150-px">
                                <div class="next-input--stylized">
                                    <input type="text" class="next-input next-input--invisible input-mask-number input-sync-item" data-target=".refund-amount-text" name="refund_amount" value="{{ $order->amount - $order->payment->refunded_amount }}">
    ...
    <div class="next-form-grid">
        <div class="next-form-grid-cell">
            <table class="refund-fields">
                <tbody>
                    <tr>
                        <td class="p-xs">
                            <div>
                                <label class="text-title-field" for="refund-note">{{ trans('plugins/ecommerce::order.payment_id') }}</label>
                                <input type="text" class="next-input next-input--invisible input-sync-item" name="payment_id" value="{{ $order->payment->charge_id }}">
                            </div>
                        </td>
                        <td class="p-xs">
                            <div>
                                <label class="text-title-field" for="refund-note">{{ trans('plugins/ecommerce::order.transaction_id') }}</label>
                                <input type="text" class="next-input next-input--invisible input-sync-item" name="trx_id" value="{{ $order->payment->trx_id }}">
                            </div>
                        </td>
                        <td class="p-xs">
                            <div>
                                <label class="text-title-field" for="refund-note">{{ trans('plugins/ecommerce::order.sku') }}</label>
                                <input type="text" class="next-input next-input--invisible input-sync-item" name="sku" value="">
                            </div>
                        </td>
                    </tr>
                </tbody>
