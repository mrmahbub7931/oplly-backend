...
@section('content')
  <div class="max-width-1200">
    <div class="ui-layout">
      <div class="flexbox-layout-sections" id="main-order-content">
        @if ($order->status == \Canopy\Ecommerce\Enums\OrderStatusEnum::CANCELED)
            ...
            <div class="wrapper-content">
              ...
              <div class="pd-all-20 p-none-t border-top-title-main">
                <div class="table-wrap">
                            ...
                            <td class="width-60-px min-width-60-px vertical-align-t">
                              <div class="wrap-img">
                                  <img class="thumb-image thumb-image-cartorderlist"
                                       src="{{ RvMedia::getImageUrl($product->owner->photo, 'thumb', false, RvMedia::getDefaultImage()) }}"
                                       alt="{{ $product->name }}">
                              </div>
                            </td>
                  ...
                  <div class="p-4 border-top-title-main">
                      <table class="table-normal table-none-border table-color-gray-text">
                          <tbody>
                          <tr>
                              <td>Request Type</td>
                              <td class="text-right pl10">{{ $order->type ?? 'Video Request' }}</td>
                          </tr>
                          <tr>
                              <td>Audience</td>
                              <td class="text-right pl10">{{ $order->target_audience }}</td>
                          </tr>
                          <tr>
                              <td>From</td>
                              <td class="text-right pl10">{{ $order->from }}</td>
                          </tr>
                          <tr>
                              <td>To</td>
                              <td class="text-right pl10">{{ $order->recepient }}</td>
                          </tr>
                          <tr>
                              <td>Occassion</td>
                              <td class="text-right pl10">{{ $order->occasion->name ?? 'none' }}</td>
                          </tr>
                          <tr>
                              <td>Instructions</td>
                              <td class="text-right pl10">{{ $order->request }}</td>
                          </tr>
                          <tr>
                              <td>Speed Service Requested</td>
                              <td class="text-right pl10">{{ $order->is_speed_service ? 'Yes' : 'No' }}</td>
                          </tr>
                          </tbody>
                      </table>
                  </div>
                  ...
                  <div class="flexbox-auto-right pl5">
                    ...
                    <tr>
                      ...
                      <td class="text-right color-subtext pl10">
                        @if ($order->payment->id)
                          <a href="{{ route('payment.show', $order->payment->id) }}" target="_blank">
                            <span>{{ format_price($order->payment->status == \Canopy\Payment\Enums\PaymentStatusEnum::COMPLETED ? $order->amount : 0) }}</span>
                          </a>
                        @else
                          <span>{{ format_price($order->payment->status == \Canopy\Payment\Enums\PaymentStatusEnum::COMPLETED ? $order->payment->amount : 0) }}</span>
                        @endif
                      </td>
                    </tr>
                    @if ($order->payment->status == \Canopy\Payment\Enums\PaymentStatusEnum::REFUNDED)
                      <tr class="hidden">
        ...
        <div class="pd-all-20 border-top-title-main">
          <div class="flexbox-grid-default flexbox-flex-wrap flexbox-align-items-center">
            ...
            @elseif ($order->payment->id)
              <div class="flexbox-auto-left">
                @if (!$order->payment->status || $order->payment->status == \Canopy\Payment\Enums\PaymentStatusEnum::PENDING)
                  <svg class="svg-next-icon svg-next-icon-size-24 svg-next-icon-gray">
                    <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#next-credit-card"></use>
                  </svg>
                @elseif ($order->payment->status == \Canopy\Payment\Enums\PaymentStatusEnum::COMPLETED || $order->payment->status == \Canopy\Payment\Enums\PaymentStatusEnum::PENDING)
              ...
              <div class="flexbox-auto-content ml15 mr15 text-upper">
                ...
                @elseif ($order->payment->status == \Canopy\Payment\Enums\PaymentStatusEnum::COMPLETED)
                  <span>{{ trans('plugins/ecommerce::order.payment_was_accepted', ['money' => format_price($order->amount - $order->payment->refunded_amount)]) }}</span>
                @elseif ($order->amount - $order->payment->refunded_amount == 0)
                  <span>{{ trans('plugins/ecommerce::order.payment_was_refunded') }}</span>
                @endif
              </div>
              @if (!$order->payment->status || in_array($order->payment->status, [\Canopy\Payment\Enums\PaymentStatusEnum::PENDING]))
                <div class="flexbox-auto-left">
              ...
              @endif
              @if ($order->payment->status == \Canopy\Payment\Enums\PaymentStatusEnum::COMPLETED && (($order->payment->amount - $order->payment->refunded_amount > 0) || ($order->products->sum('qty') - $order->products->sum('restock_quantity') > 0)))
                <div class="flexbox-auto-left">
        ...
        <div class="pd-all-20 border-top-title-main">
          <div class="flexbox-grid-default flexbox-flex-wrap flexbox-align-items-center">
            @if ($order->status != \Canopy\Ecommerce\Enums\OrderStatusEnum::RELEASED && $order->status != 'completed' || $order->video == '')
              <div class="flexbox-auto-left">
              ...
              <div class="flexbox-auto-content ml15 mr15 text-upper">
                <span>VIDEO DELIVERABLE WILL BE HERE</span>
              </div>
            ...
            @else
                ...
                <div class="flexbox-auto-left">
                  <div class="item">
                    <button class="btn btn-primary btn-trigger-complete" data-target="{{ route('orders.complete-order', $order->id) }}">Accept</button>
                  </div>

                  {{-- <div class="item">
                    <button class="btn btn-primary btn-trigger-reject" data-target="{{ route('orders.get-shipment-form', $order->id) }}">{{ trans('plugins/ecommerce::order.delivery') }}</button>
                  </div> --}}
              ...
              <div class="video-create-wrap @if ($order->status != \Canopy\Ecommerce\Enums\OrderStatusEnum::PROCESSING && $order->status != 'completed' || $order->video == '') hidden @endif">
                ...
                <div class="text-center p-3">
                  <a href="{{ route('orders.download-video', $order->id) }}" class="btn btn-info">
              ...
              <div class="pd-all-20 border-top-title-main">
                <div class="flexbox-grid-default flexbox-flex-wrap flexbox-align-items-center">
                  @if ($order->status == \Canopy\Ecommerce\Enums\OrderStatusEnum::CANCELED && !$order->shipment->id)
                    <div class="flexbox-auto-left">
                      <svg class="svg-next-icon svg-next-icon-size-20 svg-next-icon-green">
                        <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#next-checkmark"></use>
                      </svg>
          ...
          <div class="ui-layout__item">
              ...
              <div class="next-card-section border-none-t">
                ...
                <ul class="ws-nm text-infor-subdued">
                  <li class="overflow-ellipsis">
                    <a class="hover-underline" href="mailto:{{ $order->user->email ? $order->user->email : $order->address->email }}">{{ $order->user->email ? $order->user->email : $order->address->email }}</a>
                  </li>
                  @if ($order->user->id)
                    <li>
            ...
            <div class="wrapper-content bg-gray-white mb20">
              <div class="pd-all-20">
                ...
                @if (!in_array($order->status, [\Canopy\Ecommerce\Enums\OrderStatusEnum::CANCELED, \Canopy\Ecommerce\Enums\OrderStatusEnum::COMPLETED]))
  ...
  {!! Form::modalAction('confirm-refund-modal', trans('plugins/ecommerce::order.refund'), 'info', view('plugins/ecommerce::orders.refund.modal', compact('order'))->render(), 'confirm-refund-payment-button', trans('plugins/ecommerce::order.refund') . ' <span class="refund-amount-text">' . format_price($order->amount - $order->payment->refunded_amount, true) . '</span>') !!}
@stop
