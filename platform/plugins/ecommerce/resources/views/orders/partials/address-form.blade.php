        ...
        <div class="list-customer-address" @if (!$isAvailableAddress) style="display: none;" @endif>
            <div class="select--arrow">
                <select name="address[address_id]" class="form-control address-control-item" id="address_id">
                    ...
                    <option value="{{ $address->id }}" @if ( ($address->is_default && !Arr::get($sessionCheckoutData, 'address_id')) ||
        ...
        <div class="row">
            <div class="col-12">
                <div class="form-group @if ($errors->has('address.name')) has-error @endif">
                    <input type="text" name="address[name]" id="address[name]" placeholder="{{ __('Full Name') }}" class="form-control address-control-item checkout-input" value="{{ old('address.name', Arr::get($sessionCheckoutData, 'name')) }}">
