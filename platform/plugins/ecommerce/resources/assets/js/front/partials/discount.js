export class DiscountManagement {
    init() {
        ...
        // coupon code apply goes here
        $(document).on('click', '.apply-coupon-code', event => {
            event.preventDefault();
            let _self = $(event.currentTarget);
            _self.find('i').remove();
            _self.html('<i class="fas fa-spin fa-spinner"></i> ' + _self.html());
            ...
            $.ajax({
                ...
                success: res => {
                    ...
                    if (!res.error) {
                        ...
                        var total = $('.total-price .raw-total-text').data('price');
                        var price = {
                            price: total,
                            target_audience: $('input[name="target_audience"]').val(),
                            delivery:  ($('.list_delivery_method').length && $('input[name="delivery_method"]').val() === 'speed') ? total * 0.4 : 0
                        }

                        $(target).load(window.location.href + '?applied_coupon=1 ' + target + ' > *', function () {
                            _self.find('i').remove();
                            $(document).trigger('discount-coupon-applied', price);
                        });
            ...
            $.ajax({
                ...
                success: res => {
                    if (!res.error) {
                        ...
                        var price = {
                            price: total,
                            target_audience: $('input[name="target_audience"]:checked').val(),
                            delivery:  ($('.list_delivery_method').length && $('input[name="delivery_method"]:checked').val() === 'speed') ? total * 0.4 : 0
                        }
                        $(target).load(window.location.href + ' ' + target + ' > *', function () {
                            _self.find('i').remove();
                            $(document).trigger('discount-coupon-removed', price);
                        });
