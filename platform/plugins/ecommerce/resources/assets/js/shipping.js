class ShippingManagement {
    init() {
        ...
        $(document).on('keyup', '.input-sync-item', event => {
            ...
            $(event.currentTarget).closest('.input-shipping-sync-wrapper').find($(event.currentTarget).data('target')).text(Canopy.numberFormat(parseFloat(number), 2));
        });
        ...
        $(document).on('keyup', '.shipping-price-district', event => {
            ...
            _self.closest('tr').find('.support-shipping .rule-adjustment-price-item').text(Canopy.numberFormat(parseFloat(basePrice) + parseFloat(adjustmentPrice), 2));
        });
        ...
        $(document).on('keyup', '.base-price-rule-item', event => {
            ...
            $.each($(document).find('.support-shipping .rule-adjustment-price-item'), (index, item) => {
                ...
                $(item).text(Canopy.numberFormat(parseFloat(basePrice) + parseFloat(adjustmentPrice)), 2);
            });
        });
    }
}
